...
```mermaid
flowchart LR
  A[样例视频] --> B[Video Analyzer]
  B --> C[Structure Extractor]
  C --> D[Technique Atoms]
  D --> E[Knowledge Store]
  F[长视频素材 + 提示词] --> G[Long Video Segmenter]
  G --> H[Slot Matcher]
...
- `packages/core`: P0 深模块，包含样例结构抽取、长视频片段候选、槽位匹配、缺口补全和结果生成。
...
- 用户上传素材只在 `UPLOAD_DIR`、`OUTPUT_DIR`、`TMP_DIR` 内处理。
- 密钥只从环境变量读取，不进入仓库。
